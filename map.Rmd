...
if(!require("showtext")) {install.packages("showtext"); library("showtext")} # show Chinese
...
data_2007$id <- countrycode(sourcevar =data_2007$country ,
origin = "country.name", destination = "iso3c")
data_2007 <- subset(data_2007, select = c("population", "id"))
dataset <- left_join(posts, data_2007, by = "id")
...
colors = c("Obtained Posts" = "red", "Total Posts" = "blue")
p2 <- ggplot() +
...
colors = c("Obtained Posts" = "red", "Chinese in 2007" = "blue")
p3 <- ggplot() +
...
p7 <- ggplot(data = user2, aes(x = date, y = location_index)) +
      geom_point(color="red") +scale_y_continuous(breaks = c(0,0.5,1), labels=yLabs)+
      xlab("Y-Whispers")+
...
```{r Plot 12.1, results='asis', message=FALSE, warning=FALSE}
# geom_smooth is at 95% confidence level
p12a <- ggplot(posts, aes(x=log(post_obtained),y=log(gdp_percapita),label = ISO3)) + 
  geom_point(shape=18, size= 3, color="blue")+
  geom_text(size=2.5, hjust=1.1, vjust=0)+
  geom_smooth(method=lm,color="blue", fill="lightblue") +
  labs(x= "Obstained Posts (log)", y = "GDP per Capita(log)")+
  theme_light() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.line = element_line(colour = "gray"))
...
```{r Plot 12.2, results='asis', message=FALSE, warning=FALSE}
# geom_smooth is at 95% confidence level
p12b <- ggplot(posts, aes(x=log(post_obtained),y=log(gdp_percapita_ppp),label = ISO3)) + 
  geom_point(shape=18, size= 3, color="blue")+
  geom_text(size=2.5, hjust=1.1, vjust=0) +
  geom_smooth(method=lm,color="blue", fill="lightblue") +
  labs(x= "Obstained Posts (log)", y = "GDP per Capita PPP(log)")+
  theme_light() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.line = element_line(colour = "gray"))
...
p14 <- ggplot(fdi_post, aes(x=log(post_obtained), y=log(as.numeric(`sum_18-20`)),
  ...
  geom_text(size=3, hjust=1.1, vjust=0)+
  geom_smooth(method=lm, color="red", fill="lightblue") +
  labs(y = "Sum of Chinese FDI 2018-2020 (log)",
       x = "Obtained Posts (log)") + 
  theme_light() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
p16 <- ggplot(labor_post, aes(x=log(post_obtained), y=log(as.numeric(`sum_18-20`)),
  ...
  geom_text(size=3, hjust=1.1, vjust=0)+
  geom_smooth(method=lm, color="red", fill="lightblue") +
  labs(y = "Sum of Labor 2018-2020 (log)",
       x = "Obtained Posts (log)") + 
  theme_light() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
p18 <- ggplot(loan_post, aes(x=log(post_obtained), y=log(as.numeric(`sum_18-19`)),
  ...
  geom_text(size=3, hjust=1.1, vjust=0)+
  geom_smooth(method=lm, color="red", fill="lightblue") +
  labs(y = "Sum of Loan 2018-2019 (log)",
       x = "Obtained Posts (log)") + 
  theme_light() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
p20 <- ggplot(trade_post, aes(x=log(post_obtained), y=log(as.numeric(`sum_18-20`)),
  ...
  geom_text(size=3, hjust=1.1, vjust=0)+
  geom_smooth(method=lm, color="red", fill="lightblue") +
  labs(y = "Sum of Trade 2018-2020 (log)",
       x = "Obtained Posts (log)") + 
  theme_light() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
### Aid Data 
```{r}
aid <- read.csv("AidDatasGlobalChineseDevelopmentFinanceDataset_v2.0.csv")
aid$id <- countrycode(sourcevar =aid$country ,
origin = "country.name", destination = "iso3c")
aid <- subset(aid, select = c("project", "id"))
aid_post <- left_join(posts, aid, by = "id")
aid_post$project[aid_post$project == 0] <- NA
```

#### Posts and aid project
```{r Plot 21, results='asis', message=FALSE, warning=FALSE}
# geom_smooth is at 95% confidence level
p21 <- ggplot(aid_post, aes(x=log(post_obtained), y=log(as.numeric(`project`)),
                            label=id)) + 
  geom_point(shape=18, size= 3, color="blue")+
  geom_text(size=3, hjust=1.1, vjust=0)+
  geom_smooth(method=lm, color="red", fill="lightblue") +
  labs(y = "Aid Projects (log)",
       x = "Obtained Posts (log)") + 
  theme_light() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
```{r results ='asis', message=FALSE, warning=FALSE}
reg8 <- lm(log(as.numeric(`project`)) ~ log(post_obtained), data= aid_post)
cov_8 <- vcovHC(reg8)
se_8 <- sqrt(diag(cov_8))

reg8a <- lm(log(as.numeric(`project`)) ~ log(total_post), data= aid_post)
cov_8a <- vcovHC(reg8a)
se_8a <- sqrt(diag(cov_8a))
```

## Descriptions of Afrobarometer {.tabset}
### Round 6  {.tabset}
```{r}
round6 <- read_sav("merged_r6_data_2016_36countries2.sav", encoding = "latin1")
round6 <- transform(round6, name = substr(RESPNO, 1, 3))
# select questions about China from round 6
round6_china <- subset(round6, select = c(RESPNO, name, Q81A, Q81B, Q81C, Q81D,
                                          Q81E))
#select obtained posts column from posts
posts_sub <- subset(posts, select = c(country, id, post_obtained, total_post))
#change the country name in order to match the name in survey
posts_sub$country[posts_sub$country == "Burundi"] <- "BDI"
posts_sub$country[posts_sub$country == "Burkina Faso"] <- "BFO"
posts_sub$country[posts_sub$country == "Ivory Coast"] <- "CDI"
posts_sub$country[posts_sub$country == "Cape Verde"] <- "CVE"
posts_sub$country[posts_sub$country == "Mali"] <- "MLI"
posts_sub$country[posts_sub$country == "Malawi"] <- "MLW"
posts_sub$country[posts_sub$country == "Niger"] <- "NGR"
posts_sub$country[posts_sub$country == "South Africa"] <- "SAF"
posts_sub$country[posts_sub$country == "Sierra Leone"] <- "SRL"
posts_sub$country[posts_sub$country == "Sao Tome and Principe"] <- "STP"
posts_sub$country[posts_sub$country == "Eswatini"] <- "SWZ"
#choose the first three characters of country and capitalize them
posts_sub <- transform(posts_sub, name = substr(country, 1, 3))
posts_sub$name <- toupper(posts_sub$name)
#merge two datasets
round6_china <- left_join(round6_china, posts_sub, by = "name")
#recode the answer
round6_china$Q81A[as.numeric(round6_china$Q81A) == "9"] <- 0
round6_china$Q81A[as.numeric(round6_china$Q81A) == "99"] <- 0
round6_china$Q81A[as.numeric(round6_china$Q81A) == "-1"] <- NA
#new column which contains ios3 and obtained posts
round6_china$id_posts <- paste(round6_china$id, "(", round6_china$post_obtained,")")
```

#### Q81A: China's influence on economy 
How much influence do you think China’s economic activities in [ENTER COUNTRY] have on our economy, or haven’t you heard enough to say?
```{r fig22, fig.height=8, fig.width=10, message=FALSE, warning=FALSE}
p22 <- ggplot(data = round6_china, aes(x = Q81A)) + 
      geom_bar(fill="blue", color="black") +
      scale_x_continuous(breaks=seq(0,3,1))+
      scale_y_continuous(expand = c(0,0))+
      facet_wrap(~ id, ncol = 4) +
      labs(title = "China's influence on economy",
           caption = "0=None / Don’t know / Haven’t heard enough/Refused to answer, 1= A little, 2= Some, 3=A lot",
           y = "",
           x= "") + 
...
```{r results ='asis', message=FALSE, warning=FALSE}
reg9 <- lm(as.numeric(`Q81A`) ~ log(post_obtained), data= round6_china)
cov_9 <- vcovHC(reg9)
se_9 <- sqrt(diag(cov_9))

reg9a <- lm(as.numeric(`Q81A`) ~ log(total_post), data= round6_china)
cov_9a <- vcovHC(reg9a)
se_9a <- sqrt(diag(cov_9a))
```

#### Q81B: China's influence: positive or negative
In general, do you think that China’s economic and political influence on [ENTER COUNTRY] is mostly positive, or mostly negative, or haven’t you heard enough to say?
```{r}
q81b <- subset(round6_china, select=c("RESPNO","Q81B","country","id",
                                      "post_obtained","total_post"))
#recode the answer
q81b$Q81B[as.numeric(q81b$Q81B) == "9"] <- 0
q81b$Q81B[as.numeric(q81b$Q81B) == "99"] <- 0
q81b$Q81B[as.numeric(q81b$Q81B) == "-1"] <- NA
```

```{r fig23, fig.height=8, fig.width=10, message=FALSE, warning=FALSE}
p23 <- ggplot(data = q81b, aes(x = Q81B)) + 
      geom_bar(fill="blue", color="black") +
      scale_x_continuous(breaks=seq(0,5,1))+
      scale_y_continuous(expand = c(0,0))+
      facet_wrap(~ id, ncol = 4) +
      labs(title = "China's influence: positive or negative",
           caption = "0=Don’t know / Haven’t heard enough/Refused to answer, 1= Very negative, 2= Somewhat negative, \n3=Neither positive nor negative, 4= Somewhat positive, 5=Very positive",
           y = "",
           x= "") + 
      theme_bw(base_size = 10) +
      theme(panel.grid.major = element_blank(),
...
```{r results ='asis', message=FALSE, warning=FALSE}
reg10 <- lm(as.numeric(`Q81B`) ~ log(post_obtained), data= q81b)
cov_10 <- vcovHC(reg10)
se_10 <- sqrt(diag(cov_10))

reg10a <- lm(as.numeric(`Q81B`) ~ log(total_post), data= q81b)
cov_10a <- vcovHC(reg10a)
se_10a <- sqrt(diag(cov_10a))
```

#### Q81C: Positive image of China
Which of the following factors contributes most to positive image of China in [ENTER COUNTRY], or haven’t you heard enough to say?
```{r}
q81c <- subset(round6_china, select=c("RESPNO","Q81C","country","id",
                                      "post_obtained","total_post"))
#recode the answer
q81c$Q81C[as.numeric(q81c$Q81C) == "9"] <- 0
q81c$Q81C[as.numeric(q81c$Q81C) == "99"] <- 0
q81c$Q81C[as.numeric(q81c$Q81C) == "-1"] <- NA
```

```{r fig24, fig.height=10, fig.width=12, message=FALSE, warning=FALSE}
p24 <- ggplot(data = q81c, aes(x = Q81C)) + 
      geom_bar(fill="blue", color="black") +
      scale_x_continuous(breaks=seq(0,7,1))+
      scale_y_continuous(expand = c(0,0))+
      facet_wrap(~ id, ncol = 4) +
      labs(title = "Postive image of China",
           caption = "0=Don’t know / Haven’t heard enough/Refused to answer, 1=China’s support in international affairs, \n2=China’s policy of non- interference in the internal affairs of African countries, \n3=China’s investment in infrastructure or other development, 4=China’s business investment, \n5=The cost of Chinese products, 6=An appreciation of the Chinese people, culture and language, 7=Some other factor",
           y = "",
           x= "") + 
      theme_bw(base_size = 10) +
      theme(panel.grid.major = element_blank(),
...
Which of the following factors contributes most to negative images of China in [ENTER COUNTRY], or haven’t you heard enough to say?
```{r}
q81d <- subset(round6_china, select=c("RESPNO","Q81D","country","id",
                                      "post_obtained","total_post"))
#recode the answer
q81d$Q81D[as.numeric(q81d$Q81D) == "9"] <- 0
q81d$Q81D[as.numeric(q81d$Q81D) == "99"] <- 0
q81d$Q81D[as.numeric(q81d$Q81D) == "-1"] <- NA
```

```{r fig25, fig.height=10, fig.width=12, message=FALSE, warning=FALSE}
p25 <- ggplot(data = q81d, aes(x = Q81D)) + 
      geom_bar(fill="blue", color="black") +
      scale_x_continuous(breaks=seq(0,7,1))+
      scale_y_continuous(expand = c(0,0))+
      facet_wrap(~ id, ncol = 4) +
      labs(title = "Negative image of China",
           caption = "0=Don’t know / Haven’t heard enough/Refused to answer, 1=China’s extraction of resources from Africa, \n2=Land grabbing by Chinese individuals or businesses, 3=China’s willingness to cooperate with undemocratic rulers in Africa, \n4=Chinese economic activities taking jobs or business, 5=The quality of Chinese products, \n6=The behavior of Chinese citizens, 7=Some other factor",
           y = "",
           x= "") + 
      theme_bw(base_size = 10) +
      theme(panel.grid.major = element_blank(),
...
In your opinion, does China’s economic development assistance to [ENTER COUNTRY] do a good job or a bad job of meeting the country’s needs, or haven’t you heard enough to say?
```{r}
q81e <- subset(round6_china, select=c("RESPNO","Q81E","country","id",
                                      "post_obtained","total_post"))
#recode the answer
q81e$Q81E[as.numeric(q81e$Q81E) == "98"] <- 0
q81e$Q81E[as.numeric(q81e$Q81E) == "99"] <- 0
q81e$Q81E[as.numeric(q81e$Q81E) == "7"] <- 0
q81e$Q81E[as.numeric(q81e$Q81E) == "9"] <- 0
q81e$Q81E[as.numeric(q81e$Q81E) == "-1"] <- NA
```

```{r fig26, fig.height=8, fig.width=10, message=FALSE, warning=FALSE}
p26 <- ggplot(data = q81e, aes(x = Q81E)) + 
      geom_bar(fill="blue", color="black") +
      scale_x_continuous(breaks=seq(0,5,1))+
      scale_y_continuous(expand = c(0,0))+
      facet_wrap(~ id, ncol = 4) +
      labs(title = "China's assistance does a good job at meeting country's needs",
           caption = "0=Don’t know / Haven’t heard enough/Refused to answer, 1=Very bad job, 2=Somewhat bad job, \n3=Neither good nor bad job, 4=Somewhat good job, 5=Very good job",
           y = "",
           x= "") + 
      theme_bw(base_size = 10) +
      theme(panel.grid.major = element_blank(),
...
```{r results ='asis', message=FALSE, warning=FALSE}
reg11 <- lm(as.numeric(`Q81E`) ~ log(post_obtained), data= q81e)
cov_11 <- vcovHC(reg11)
se_11 <- sqrt(diag(cov_11))

reg11a <- lm(as.numeric(`Q81E`) ~ log(total_post), data= q81e)
cov_11a <- vcovHC(reg11a)
se_11a <- sqrt(diag(cov_11a))
```

### Round 8  {.tabset}
```{r}
#round8 <- read.csv("round8.csv")
#round8_china <- subset(round8, select = c(RESPNO, Q65A, Q65B, Q65C, Q65D,
#                                          Q69B, Q70E))
```


## Regression tables {.tabset}
### From Afrobarometer-Post
...
### From hopkins_data & Aid data
```{r  Table 3, results ='asis', message=FALSE, warning=FALSE}
stargazer(reg4,reg4a,reg5,reg5a,reg6,reg6a,reg7,reg7a,reg8,reg8a, type="html",
          dep.var.labels = c("","","","",""),
          column.labels = c("Chinese FDI<br>(log)", "Labor<br>(log)", 
                            "Loan<br>(log)", "Trade<br>(log)", "Aid Project<br>(log)"),
          covariate.labels= c("Obatined Posts(log)", "Total Posts(log)"),
          se=list(se_4,se_4a,se_5,se_5a,se_6,se_6a,se_7,se_7a,se_8,se_8a), 
...
### Afrobarometer Survey {.tabset}
```{r message=FALSE, warning=FALSE}
library(MASS)
reg9b <- polr(as.factor(`Q81A`) ~ log(post_obtained), data= round6_china, Hess=TRUE)
reg9c <- polr(as.factor(`Q81A`) ~ log(total_post), data= round6_china, Hess=TRUE)
reg10b <- polr(as.factor(`Q81B`) ~ log(post_obtained), data= q81b, Hess=TRUE)
reg10c <- polr(as.factor(`Q81B`) ~ log(total_post), data= q81b, Hess=TRUE)
reg11b <- polr(as.factor(`Q81E`) ~ log(post_obtained), data= q81e, Hess=TRUE)
reg11c <- polr(as.factor(`Q81E`) ~ log(total_post), data= q81e, Hess=TRUE)
```

#### Q81A: China's influence on economy
```{r Table 4, message=FALSE, warning=FALSE, results='asis'}
tab_model(reg9,reg9a,reg9b,reg9c, p.style = "stars", show.ci = FALSE, collapse.se = TRUE, 
          pred.labels = c("Intercept", "Obatined Posts(log)","Total Posts(log)", "None|A little", "A little|Some", "Some|A lot"), 
          title = "China's influence on economy(se in parentheses)",
          dv.labels = c("","","",""))
```

#### Q81B: China's influence: positive or negative
```{r Table 5, message=FALSE, warning=FALSE, results='asis'}
tab_model(reg10,reg10a,reg10b,reg10c, p.style = "stars", show.ci = FALSE, collapse.se = TRUE, 
          pred.labels = c("Intercept", "Obatined Posts(log)","Total Posts(log)", 
                          "None|Very negative", "Very negative|Somewaht negative", 
                          "Somewaht negative|Neutral", "Neutral|Somewhat positive",
                          "Somewhat positive|Very positive"), 
          title = "China's influence: positive or negative(se in parentheses)",
          dv.labels = c("","","",""))
```

#### Q81E: China's assistance does a good job at meeting country's needs
```{r Table 6, message=FALSE, warning=FALSE, results='asis'}
tab_model(reg11,reg11a,reg11b,reg11c, p.style = "stars", show.ci = FALSE, collapse.se = TRUE, 
          pred.labels = c("Intercept", "Obatined Posts(log)","Total Posts(log)", 
                          "None|Very bad", "Very bad|Somewaht bad", 
                          "Somewaht bad|Neutral", "Neutral|Somewhat good",
                          "Somewhat good|Very good"), 
          title = "China's assistance does a good job at meeting country's needs(se in parentheses)",
          dv.labels = c("","","",""))
```
